...
final class TransactionWithdrawalOverviewWidget extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $withdrawalTrend = Trend::query(Transaction::query()
            ->withdrawal()
            ->approved())
        ...
        return [
            Stat::make('withdrawal_transaction_stats', Number::format($todayTotal))
                ...
                ->chart(
                    $withdrawalTrend->map(fn(TrendValue $value) => (int) $value->aggregate)->toArray(),
                )
                ->color('primary'),
        ];
    }
