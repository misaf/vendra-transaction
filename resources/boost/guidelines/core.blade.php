...
- The ledger is the single source of balance truth: wallet `balance` is a cache written only by `Actions\PostLedgerEntryAction` under a row lock, every movement is an immutable `LedgerEntry` with a `balance_after` snapshot, and `vendra-transaction:verify-balances` recomputes and optionally repairs drift. Never update `wallets.balance` or ledger rows directly.
