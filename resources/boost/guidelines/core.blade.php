...
- The ledger is the single source of balance truth: wallet `balance` is a cache written only under a row lock, by `Actions\PostLedgerEntryAction` or `Actions\RepairWalletBalanceAction`, every movement is an immutable `LedgerEntry` with a `balance_after` snapshot, and `vendra-transaction:verify-balances` recomputes and, with `--repair`, fixes drift through `RepairWalletBalanceAction`. Never update `wallets.balance` or ledger rows directly.
...
- Stay decoupled from concrete user and currency modules: resolve the user model through `Support\TransactionUsers::model()` (`auth.providers.users.model`) and currency defaults/options through Support's `CurrencyIntegration`. Wallets persist a scalar `currency_code`; never import `Misaf\VendraUser` or `Misaf\VendraCurrency`. A host may install `misaf/vendra-currency` as the optional managed currency provider.
- Gateways are admin-managed labels only (translatable `name`/`description`, scalar `slug`, media logo, sortable). Payment processing logic does not belong in this module. Resolve gateways by scalar slug via `TransactionGatewayRegistry`; the internal gateway slug is `TransactionGatewayRegistry::INTERNAL_GATEWAY_SLUG`. Wallets come from `WalletResolver`, and a wallet's limit from `$wallet->limitFor()`.
...
- Use nullable `transactions.idempotency_key` for retry-safe creation. `CreateTransactionAction::execute()` must return the original transaction for an identical key and financial payload, and reject reuse for different details. Keep it in the consolidated create migration and its package stub; do not add a follow-up alteration migration.
