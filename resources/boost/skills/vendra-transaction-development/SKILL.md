...
Treat `app-modules/vendra-transaction` as the source of transaction domain behavior and Filament admin UI.
...
- Run module checks from the package when possible: `composer --working-dir=app-modules/vendra-transaction test` and `composer --working-dir=app-modules/vendra-transaction analyse`.
